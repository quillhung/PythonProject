...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    # If the name is not in the name_data dict, create a new dict to store it.
    if name not in name_data:
        name_data[name] = {}
    # If the year is not in the name dict, add the year and rank to the name dict.
    if year not in name_data[name]:
        name_data[name][year] = rank
    # If the name dict already has the year, update the rank if the new rank is better.
